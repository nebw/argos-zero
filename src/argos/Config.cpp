...
namespace config {
    ...
    std::string usage(const char *programPath, const po::options_description &optional,
    ...
    Config parse(int argc, const char **argv) {
        ...
        required.add_options()("networkPath,n", po::value<std::string>()->required(),
                               "path to trained model");

        // TODO: add better params description
        optional.add_options()("help,h", "print this message and exit")(
            "config,c", po::value<std::string>(), "path to config file")(
            "logfilePath,l", po::value<std::string>(),
            "path where log file should be stored\n"
            "by default logs are stored in `<timestamp>.log` file in the same directory as "
            "network")("deviceType,d", po::value<std::string>(),
                       "set device type (CPU, GPU, CPU_PINNED)")(
            ...
            "time-c", po::value<int>(), "set time c")("time-maxPly", po::value<int>(),
                                                      "set maxPly")(
            "time-delay", po::value<int>(), "set delay")("engine-totalTime", po::value<int>(),
                                                         "set totalTime in milliseconds")(
            "engine-selfplayRollouts", po::value<size_t>(), "set selfplayRollouts")(
        ...
        try {
            ...
            if (vm.count("help")) {
                std::cout << usage(argv[0], optional, positional) << std::endl
                          << options << std::endl;
                exit(EXIT_SUCCESS);
            }
            ...
            std::cout << usage(argv[0], optional, positional) << std::endl << options << std::endl;
            exit(EXIT_FAILURE);
        } catch (po::error &e) {
        ...
        if (vm.count("config")) {
            ...
            if (fs::exists(configPath)) {
                ...
                try {
                    ...
                    treeBuilder.numEvaluationThreads(
                        treeParser.get<size_t>("num_evaluation_threads"));
                    treeBuilder.numThreads(treeParser.get<size_t>("num_threads"));
                    treeBuilder.randomizeFirstNMoves(
                        treeParser.get<size_t>("randomize_first_n_moves"));
                    ...
                    engineBuilder.selfplayRollouts(engineParser.get<size_t>("selfplay_rollouts"));
                    engineBuilder.resignThreshold(engineParser.get<float>("resign_threshold"));
                    engineBuilder.totalTime(
                        std::chrono::milliseconds(engineParser.get<size_t>("total_time")));
        ...
        if (vm.count("tree-numEvaluationThreads"))
            treeBuilder.numThreads(vm["tree-numEvaluationThreads"].as<size_t>());
        /* tree-numThreads defined above */
        if (vm.count("tree-randomizeFirstNMoves"))
            treeBuilder.randomizeFirstNMoves(vm["tree-randomizeFirstNMoves"].as<size_t>());
        if (vm.count("tree-numLastRootNodes"))
            treeBuilder.numLastRootNodes(vm["tree-numLastRootNodes"].as<size_t>());
        if (vm.count("tree-virtualPlayouts"))
            treeBuilder.virtualPlayouts(vm["tree-virtualPlayouts"].as<size_t>());
        ...
        if (vm.count("tree-trainingMode"))
            treeBuilder.trainingMode(vm["tree-trainingMode"].as<bool>());
        if (vm.count("time-c")) timeBuilder.C(vm["time-c"].as<int>());
        if (vm.count("time-maxPly")) timeBuilder.maxPly(vm["time-maxPly"].as<int>());
        if (vm.count("time-delay"))
            timeBuilder.delay(std::chrono::milliseconds(vm["time-delay"].as<int>()));
        if (vm.count("engine-totalTime"))
            engineBuilder.totalTime(std::chrono::milliseconds(vm["engine-totalTime"].as<int>()));
        if (vm.count("engine-selfplayRollouts"))
            engineBuilder.selfplayRollouts(vm["engine-selfplayRollouts"].as<size_t>());
        if (vm.count("engine-resignThreshold"))
            engineBuilder.resignThreshold(vm["engine-resignThreshold"].as<float>());

        auto config = configBuilder.tree(treeBuilder.build())
        ...
        cout << "config.engine.selfplayRollouts: " << config.engine.selfplayRollouts << endl;
