...
namespace config {
    namespace defaults {
        ...
        namespace time {
            static const constexpr std::chrono::milliseconds DEFAULT_DELAY =
                std::chrono::milliseconds(10);
        }
        namespace engine {
            static const constexpr std::chrono::milliseconds DEFAULT_TOTAL_TIME =
                std::chrono::milliseconds(1000 * 60 * 10);
        }
    }  // namespace defaults
    ...
    private:
        explicit Tree(const size_t &batchSize, const size_t &numEvaluationThreads,
    ...
    public:
        Builder batchSize(const size_t &val) {
            _batchSize = val;
            return *this;
        }
        Builder numEvaluationThreads(const size_t &val) {
            _numEvaluationThreads = val;
            return *this;
        }
        Builder numThreads(const size_t &val) {
            _numThreads = val;
            return *this;
        }
        Builder randomizeFirstNMoves(const size_t &val) {
            _randomizeFirstNMoves = val;
            return *this;
        }
        Builder numLastRootNodes(const size_t &val) {
            _numLastRootNodes = val;
            return *this;
        }
        Builder virtualPlayouts(const size_t &val) {
            _virtualPlayouts = val;
            return *this;
        }
        Builder expandAt(const size_t &val) {
            _expandAt = val;
            return *this;
        }
        Builder betaPrior(const float &val) {
            _betaPrior = val;
            return *this;
        }
        Builder priorC(const float &val) {
            _priorC = val;
            return *this;
        }
        ...
        Builder trainingMode(const bool &val) {
            _trainingMode = val;
            return *this;
        }
        Tree build() {
    ...
    private:
        explicit Time(const int &C, const int &maxPly, const std::chrono::milliseconds &delay)
            : C(C), maxPly(maxPly), delay(delay) {}
    ...
    public:
        Builder C(const int &C) {
            _C = C;
            return *this;
        }
        Builder maxPly(const int &maxPly) {
            _maxPly = maxPly;
            return *this;
        }
        Builder delay(const std::chrono::milliseconds &delay) {
            _delay = delay;
            return *this;
        }
        Time build() { return Time(_C, _maxPly, _delay); }
    };

    class Engine final {
    public:
        static const constexpr float DEFAULT_RESIGN_THRESHOLD = 0.1f;
        static const constexpr size_t DEFAULT_SELFPLAY_ROLLOUTS = 1000;

    private:
        explicit Engine(const float &resignThreshold, const size_t selfplayRollouts,
                        const std::chrono::milliseconds &totalTime)
            : resignThreshold(resignThreshold),
              selfplayRollouts(selfplayRollouts),
              totalTime(totalTime) {}

    public:
        class Builder;
        const float resignThreshold;
        const size_t selfplayRollouts;
        const std::chrono::milliseconds totalTime;
    };
    ...
    private:
        ...
        size_t _selfplayRollouts = DEFAULT_SELFPLAY_ROLLOUTS;
        std::chrono::milliseconds _totalTime = defaults::engine::DEFAULT_TOTAL_TIME;

    public:
        Builder resignThreshold(const float &resignThreshold) {
            _resignThreshold = resignThreshold;
            return *this;
        }
        Builder selfplayRollouts(const float &selfplayRollouts) {
            _selfplayRollouts = selfplayRollouts;
            return *this;
        }
        Builder totalTime(const std::chrono::milliseconds &totalTime) {
            _totalTime = totalTime;
            return *this;
        }
        Engine build() { return Engine(_resignThreshold, _selfplayRollouts, _totalTime); }
    };
    ...
    private:
        explicit Config(const boost::filesystem::path &networkPath,
                        const boost::filesystem::path &logFilePath, const Tree &tree, Time time,
                        Engine engine, const size_t &boardSize, const MXNET_DEVICE_TYPE &deviceType,
                        const int &port, const std::string &server)
    ...
    public:
        Builder(const boost::filesystem::path &networkPath,
                const boost::filesystem::path &logFilePath)
            : _networkPath(networkPath), _logFilePath(logFilePath) {}
        Builder tree(const Tree &tree) {
            _tree = std::make_shared<Tree>(tree);
            return *this;
        }
        Builder time(const Time &time) {
            _time = std::make_shared<Time>(time);
            return *this;
        }
        Builder engine(const Engine &engine) {
            _engine = std::make_shared<Engine>(engine);
            return *this;
        }
        Builder boardSize(const size_t &boardSize) {
            _boardSize = boardSize;
            return *this;
        }
        Builder deviceType(const MXNET_DEVICE_TYPE &deviceType) {
        ...
        Builder server(const std::string &server) {
            _server = server;
            return *this;
        }
        Config build() {
            return Config(_networkPath, _logFilePath, _tree ? *_tree : DEFAULT_TREE,
                          _time ? *_time : DEFAULT_TIME, _engine ? *_engine : DEFAULT_ENGINE,
                          _boardSize, _deviceType, _port, _server);
        }
    };
...
namespace tree {
    static const size_t batchSize =
        argos::config::Tree::DEFAULT_BATCH_SIZE;  // TODO used in array initialization...
}  // namespace tree
}  // namespace config
