...
#ifdef _MSC_VER
#define EGO_FAIL(msg) (TestFail(msg, __FILE__, __LINE__, __FUNCSIG__), exit(0))
#else
#define EGO_FAIL(msg) (TestFail(msg, __FILE__, __LINE__, __PRETTY_FUNCTION__), exit(0))
#endif

// Always evaluate condition and always assert true.
#define CHECK2(expr, block)  \
    {                        \
        if (!(expr)) {       \
            block;           \
            EGO_FAIL(#expr); \
...
#define EGO_CHECK(expr) CHECK2(expr, {})

// Evalueate and assert only if kCheckAsserts == true
#define ASSERT2(expr, block)                       \
    {                                              \
