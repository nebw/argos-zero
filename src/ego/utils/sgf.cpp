...
string SgfNodeProperties::get_comment() {
    ...
    for (list<string>::iterator comment = comments.begin(); comment != comments.end(); comment++) {
        out << *comment << endl;
    }
    return out.str();
}
...
#define add_moves(prop_name)                                         \
    {                                                                \
        ...
        for (list<string>::iterator sgf_coord = prop_values.begin(); \
             sgf_coord != prop_values.end(); sgf_coord++) {          \
            ret.push_back(Vertex::OfSgfString(*sgf_coord));          \
        }                                                            \
    }
...
list<Vertex> SgfNodeProperties::get_vertices_to_play(Player pl) {
    return get_vertices_to_play(Color::OfPlayer(pl));
}

string SgfNodeProperties::get_single_property(string prop_name, string default_value) {
...
uint SgfNodeProperties::get_board_size() { return atoi(get_single_property("SZ", "19").data()); }

float SgfNodeProperties::get_komi() { return atof(get_single_property("KM", "0.0").data()); }
...
string SgfNode::to_sgf_string(bool print_braces) {
    ...
    for (list<SgfNode>::iterator child = children.begin(); child != children.end(); child++)
...
#define return_fail      \
    ...
    while (in >> c) {
        switch (c) {
            ...
            case '[':
                ...
                current->properties.add(prop_name, prop_value);  // we reuse last property, this may
                                                                 // be nonlocal in nonvalid sgf
                                                                 // files
    ...
    if (cb_stack.empty() == false) return_fail;
...
string SgfTree::to_sgf_string() {
    ostringstream out;
    for (list<SgfNode>::iterator child = root->children.begin(); child != root->children.end();
         child++)
...
SgfNodeProperties& SgfTree::properties() { return game_node()->properties; }
