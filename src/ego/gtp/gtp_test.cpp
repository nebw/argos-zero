...
struct Fixture {
    ...
    Fixture() : gtp_user(gtp), f(1.5), i(-1), s("GTP rulez") {
        gtp.Register("+", CAdd);
...
BOOST_AUTO_TEST_CASE(BuiltInCommands) {
    ...
    expected_out << expected_help_msg.str() << expected_help_msg.str() << "? too many parameters"
                 << endl
                 << endl
                 << "= true" << endl
                 << endl
...
BOOST_AUTO_TEST_CASE(RunSingleCommand) {
    ...
    BOOST_CHECK_EQUAL(gtp.RunOneCommand("please", &response), Gtp::Repl::Failure);
...
BOOST_AUTO_TEST_CASE(NoDefaultConstructorGtp) {
    ...
    BOOST_CHECK_EQUAL(gtp.RunOneCommand("read_pdc 42 istheanswer", &response), Gtp::Repl::Success);
    BOOST_CHECK_EQUAL(response, "istheanswer - 42");

    BOOST_CHECK_EQUAL(gtp.RunOneCommand("read_pdc istheanswer 42", &response), Gtp::Repl::Failure);
