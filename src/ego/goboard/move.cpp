...
Move::Move(Player player, Vertex vertex) : Nat<Move>(player.GetRaw() | (vertex.GetRaw() << 1)) {
...
Move Move::OtherPlayer() const { return Move::OfRaw(GetRaw() ^ 0x1); }

Player Move::GetPlayer() const { return Player::OfRaw(GetRaw() & 0x1); }

Vertex Move::GetVertex() const { return Vertex::OfRaw(GetRaw() >> 1); }

string Move::ToGtpString() const {
    return GetPlayer().ToGtpString() + " " + GetVertex().ToGtpString();
}
