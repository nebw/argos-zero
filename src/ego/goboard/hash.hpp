...
class Hasher {
public:
    size_t operator()(Hash const& key) const { return key.hash; }
};
...
public:
    ...
    Hash OfKoVertex(Vertex v) const;
    Hash OfPlayer(Player p) const;

private:
    NatMap<Move, Hash> hashes;
    NatMap<Vertex, Hash> koHashes;
    NatMap<Player, Hash> playerHashes;
};
...
public:
    ...
    static Hash3x3 Any() { return OfRaw(0xfffff); }
    ...
    static Hash3x3 OfBoard(const NatMap<Vertex, Color>& color_at, Vertex v) {
        ...
        uint raw = 0;
        ForEachNat(Dir, dir) { raw |= color_at[v.Nbr(dir)].GetRaw() << (2 * dir.GetRaw()); }

        return OfRaw(raw);
    }

    Color ColorAt(Dir dir) const { return Color::OfRaw((GetRaw() >> (2 * dir.GetRaw())) & 3); }
    ...
    void ResetAtariBits() { raw &= (1 << 16) - 1; }
    ...
    bool IsEyelike(Player pl) const {
        ...
        if (color_cnt[Color::OfPlayer(pl)] + color_cnt[Color::OffBoard()] < 4) { return false; }

        return diag_color_cnt[Color::OfPlayer(pl.Other())] +
                   (diag_color_cnt[Color::OffBoard()] > 0) <
               2;
    }
    ...
    Hash3x3 Rotate90() const {
        uint raw90_colors = ((raw << 2) & 0x0fcfc) | ((raw >> 6) & 0x00303);  // new N and NW

        uint raw90_ataris = ((raw << 1) & 0xe0000) | ((raw >> 3) & 0x10000);  // new N and NW
    ...
    Hash3x3 Mirror() const {
        ...
        ret.ResetAtariBits();
        ret.SetAtariBits(IsInAtari(Dir::N()), IsInAtari(Dir::W()), IsInAtari(Dir::S()),
                         IsInAtari(Dir::E()));
        return ret;
    }
    ...
    string ToAsciiArt() const {
        ostringstream out;
        out << ColorAt(Dir::NW()).ToShowboardChar() << " " << ColorAt(Dir::N()).ToShowboardChar()
            << (IsInAtari(Dir::N()) ? "!" : " ") << ColorAt(Dir::NE()).ToShowboardChar() << " "
            << endl
            << ColorAt(Dir::W()).ToShowboardChar() << (IsInAtari(Dir::W()) ? "!" : " ") << "."
            << " " << ColorAt(Dir::E()).ToShowboardChar() << (IsInAtari(Dir::E()) ? "!" : " ")
            << endl
            << ColorAt(Dir::SW()).ToShowboardChar() << " " << ColorAt(Dir::S()).ToShowboardChar()
            << (IsInAtari(Dir::S()) ? "!" : " ") << ColorAt(Dir::SE()).ToShowboardChar() << " "
            << endl;
